...
const AdminLists: React.FC<AdminListsProps> = ({ proData, onSavePro }) => {
  ...
  const activeUnit = Unit.HAM; // Unidade Fixa
  ...
  const handleConfirmImport = async () => {
    ...
    setSyncState({ isOpen: true, status: 'processing', title: 'Sincronizando', message: 'Gravando dados em Manaus...' });
    ...
    try {
        ...
        setSyncState({ isOpen: true, status: 'success', title: 'Sincronização HAM', message: `Sucesso!\n\nNovos/Atualizados: ${stats.updated + stats.new}\nInativos: ${stats.deactivated}` });
        setPreviewData([]); 
    } catch (e: any) {
    ...
    <div className="space-y-12">
      ...
      <section className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-6">
          <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tighter">Importação HAM (Manaus)</h2>
          <div className="bg-blue-50 px-6 py-2 rounded-xl text-[10px] font-black uppercase text-blue-600 border border-blue-100">Unidade Manaus (Ativa)</div>
        </div>
        ...
        <div className="bg-slate-50 p-6 rounded-[2rem] flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            {previewData.length > 0 && (
                <button onClick={handleConfirmImport} className="px-8 py-3 bg-emerald-500 text-white rounded-xl font-black text-[10px] uppercase shadow-lg shadow-emerald-200 hover:bg-emerald-600 transition-all flex items-center gap-2">
                    <i className="fas fa-sync"></i> Sincronizar HAM
                </button>
            )}
        </div>
